...
ARG TELEMETRY_TOKEN=vAyMmhLjnNcPrQDXEpo9
...
# Build cmd
RUN CGO_ENABLED=0 GOOS=${TARGETOS} GOARCH=${TARGETARCH} GO111MODULE=on go build -mod vendor -ldflags "-X github.com/loft-sh/vcluster/pkg/telemetry.SyncerVersion=$BUILD_VERSION -X github.com/loft-sh/vcluster/pkg/telemetry.TelemetryToken=$TELEMETRY_TOKEN" -o /vcluster cmd/vcluster/main.go
