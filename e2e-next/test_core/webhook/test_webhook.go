	...
	return Describe("AdmissionWebhook",
		...
		func() {
			...
			It("should be able to deny attaching to a pod", func(ctx context.Context) {
				...
				By("Attempting to attach to the pod via kubectl (should be denied by webhook)", func() {
					// Use kubectl attach which properly triggers the Connect admission webhook.
					// The REST client and SPDY executor don't reliably trigger admission for
					// Connect operations through the background proxy.
					currentCluster := cluster.CurrentClusterNameFrom(ctx)
					kubeconfigPath := cluster.From(ctx, currentCluster).GetKubeconfig()

					cmd := exec.CommandContext(ctx, "kubectl", "attach",
						"--kubeconfig", kubeconfigPath,
						fmt.Sprintf("--namespace=%s", ns),
						podName, "-i", "-c=container1")
					output, err := cmd.CombinedOutput()
					Expect(err).To(HaveOccurred(), "kubectl attach should be denied by webhook")
					Expect(string(output)).To(ContainSubstring("attaching to pod"),
						"expected denial message about attaching to pod, got: %s", string(output))
				})
			})
		},
	)
