...
var _ = Describe("Secrets sync from host",
	...
	func() {
		var (
			...
			vClusterName   = clusters.FromHostSecretsVClusterName
		)
		...
		It("syncs secrets from wildcard namespace and propagates updates", func(ctx context.Context) {
			...
			By("creating from-host-sync-test-2 namespace on host")
			ensureNamespace(ctx, hostNS)

			By("creating secret (dummy) in from-host-sync-test-2 namespace on host")
			...
			By("waiting for secret to be synced to barfoo2 namespace in vcluster")
			...
			By("updating the host secret with new data, labels, and annotations")
			...
			By("waiting for the update to propagate to vcluster")
			Eventually(func(g Gomega) {
		...
		It("syncs exact-name secret from default namespace to vcluster", func(ctx context.Context) {
			...
			By("creating my-secret in default namespace on host")
			_, err := hostClient.CoreV1().Secrets(hostNS).Create(ctx, &corev1.Secret{
				ObjectMeta: metav1.ObjectMeta{
					Name:      hostSecretName,
					Namespace: hostNS,
			...
			By("waiting for secret to be synced as secret-my to barfoo2 namespace in vcluster")
			Eventually(func(g Gomega) {
		...
		It("syncs secret from vcluster host namespace to configured virtual namespace", func(ctx context.Context) {
			// /my-secret-in-default-ns -> barfoo2/secret-from-default-ns is a fixed mapping
			...
			By("creating my-secret-in-default-ns in vcluster host namespace on host")
			...
			By("waiting for secret to be synced as secret-from-default-ns to barfoo2 namespace in vcluster")
			Eventually(func(g Gomega) {
				secret, err := vClusterClient.CoreV1().Secrets(virtualNS).Get(ctx, virtualSecretName, metav1.GetOptions{})
				g.Expect(err).NotTo(HaveOccurred())
				g.Expect(secret.Data).To(Equal(map[string][]byte{
		...
		It("deleting secret on host also deletes it from vcluster", func(ctx context.Context) {
			...
			By("creating the deletion-test-secret in from-host-sync-test-2 on host")
			_, err := hostClient.CoreV1().Secrets(hostNS).Create(ctx, &corev1.Secret{
				ObjectMeta: metav1.ObjectMeta{
					Name:      secretName,
					Namespace: hostNS,
		...
		It("synced secrets from multiple sources can be used as env source for a pod in vcluster", func(ctx context.Context) {
			...
			By("creating from-host-sync-test-3 namespace on host")
			ensureNamespace(ctx, hostNS)

			By("creating first secret with env data on host")
			_, err := hostClient.CoreV1().Secrets(hostNS).Create(ctx, &corev1.Secret{
				ObjectMeta: metav1.ObjectMeta{
					Name:      secret1Name,
					Namespace: hostNS,
			...
			By("creating second secret with additional env data on host")
			_, err = hostClient.CoreV1().Secrets(hostNS).Create(ctx, &corev1.Secret{
				ObjectMeta: metav1.ObjectMeta{
					Name:      secret2Name,
					Namespace: hostNS,
			...
			By("waiting for both secrets to be synced to barfoo2 namespace in vcluster")
			Eventually(func(g Gomega) {
			...
			By("waiting for the default service account to be ready in barfoo2 namespace")
			Eventually(func(g Gomega) {
			...
			By("creating the pod in vcluster")
			...
			By("waiting for the pod to reach Running phase in vcluster")
			Eventually(func(g Gomega) {
			...
			By("exec-ing printenv in the pod to verify env vars from both synced secrets")
