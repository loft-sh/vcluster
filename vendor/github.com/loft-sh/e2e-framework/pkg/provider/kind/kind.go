...
func (k *Cluster) UnmarshalJSON(data []byte) error {
	...
	k.name = clusterData.Name
	k.path = clusterData.Path
	k.version = clusterData.Version
	k.image = clusterData.Image
	k.kubecfgFile = clusterData.KubecfgFile

	// Initialize the rest.Config if kubecfgFile is available
	if k.kubecfgFile != "" {
		if err := k.initKubernetesAccessClients(); err != nil {
...
func (k *Cluster) MarshalJSON() ([]byte, error) {
	return json.Marshal(clusterJSON{
		Type:        Type,
		Path:        k.path,
		Name:        k.name,
		KubecfgFile: k.kubecfgFile,
		Version:     k.version,
		Image:       k.image,
	})
}
