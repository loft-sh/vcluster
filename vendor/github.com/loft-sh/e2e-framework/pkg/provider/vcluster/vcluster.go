...
func WithLocalChartDir(localChartDir string) support.ClusterOpts {
	return func(c support.E2EClusterProvider) {
		v, ok := c.(*Cluster)
		if ok {
			v.localChartDir = localChartDir
		}
	}
}

func WithBackgroundProxyImage(backgroundProxyImage string) support.ClusterOpts {
	return func(c support.E2EClusterProvider) {
		v, ok := c.(*Cluster)
		if ok {
			v.backgroundProxyImage = backgroundProxyImage
		}
	}
}
...
// clusterJSON is a struct for JSON serialization/deserialization of Cluster.
...
func (c *Cluster) UnmarshalJSON(data []byte) error {
	var clusterData clusterJSON
	if err := json.Unmarshal(data, &clusterData); err != nil {
		return fmt.Errorf("failed to unmarshal cluster data: %w", err)
	}

	// Create a new Cluster instance
	c.name = clusterData.Name
	c.namespace = clusterData.Namespace
	c.path = clusterData.Path
	c.kubecfgFile = clusterData.KubecfgFile
	...
	if c.kubecfgFile != "" {
		if err := c.initKubernetesAccessClients(); err != nil {
			return fmt.Errorf("failed to initialize kubernetes access clients: %w", err)
		}
	}
...
func (c *Cluster) getKubeconfig(args ...string) (string, error) {
	...
	command := fmt.Sprintf(`%s connect %s --print %s`, c.path, c.name, strings.Join(args, " "))
	echo := gexe.New()
	if c.hostKubeCfg != "" {
		echo = echo.SetVar("kubecfg", c.hostKubeCfg)
		command = `/bin/sh -c "KUBECONFIG=${kubecfg} ` + command + `"`
	}
	p := echo.NewProc(command)
	p.SetStdout(&stdout)
	p.SetStderr(&stderr)
