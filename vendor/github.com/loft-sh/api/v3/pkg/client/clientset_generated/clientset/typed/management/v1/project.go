...
func (c *projects) ImportVirtualCluster(ctx context.Context, projectName string, projectImportVirtualCluster *v1.ProjectImportVirtualCluster, opts metav1.CreateOptions) (result *v1.ProjectImportVirtualCluster, err error) {
	result = &v1.ProjectImportVirtualCluster{}
	err = c.client.Post().
		Resource("projects").
		Name(projectName).
		SubResource("importvirtualcluster").
		VersionedParams(&opts, scheme.ParameterCodec).
		Body(projectImportVirtualCluster).
		Do(ctx).
		Into(result)
	return
}
