...
var usage = fmt.Sprintf(`Usage:
  go run -mod vendor ./hack/assets/cmd/main.go [v]X.Y.Z [--latest] [--optional] [--pro]
  go run -mod vendor ./hack/assets/cmd/main.go [v]X.Y.Z [--kubernetes-distro <%s>] [--kubernetes-version X.Y.Z] [--pro]
...
func Main() {
	...
	proOnly := pflag.Bool("pro", false, "Only add the pro image of vcluster")
	...
	images := GetImages(cleanTag, *latest, *proOnly, *optional, *kubernetesVersion, *kubernetesDistro)
...
func GetImages(cleanTag string, latest bool, proOnly bool, optional bool, kubernetesVersion string, kubernetesDistro string) []string {
	var images []string
	if !optional {
		images = GetVclusterProImages(cleanTag)
		if !proOnly {
			images = append([]string{"ghcr.io/loft-sh/vcluster:" + cleanTag}, images...) // Add oss image
		}
	}
...
// GetVclusterProImages returns a list of vcluster images
func GetVclusterProImages(cleanTag string) []string {
	return UniqueAppend(nil,
