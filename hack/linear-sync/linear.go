...
// IssueState returns the current state of the issue.
func (l *LinearClient) IssueState(ctx context.Context, issueID string) (string, error) {
	...
	return query.Issue.State.Id, nil
}
...
func (l *LinearClient) MoveIssueToState(ctx context.Context, dryRun bool, issueID, releasedStateID, releaseTagName, releaseDate string) error {
	// (ThomasK33): Skip CVEs
	if strings.HasPrefix(strings.ToLower(issueID), "cve") {
		return nil
	}

	currentIssueState, err := l.IssueState(ctx, issueID)
	if err != nil {
	...
	if currentIssueState == releasedStateID {
