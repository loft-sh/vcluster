...
func getInstallStandaloneScript(ctx context.Context, vClusterVersion string, globalFlags *flags.GlobalFlags) ([]byte, error) {
	if path := os.Getenv(installStandaloneScriptEnv); path != "" {
		...
		if err := validateInstallStandaloneScript(script); err != nil {
			return nil, fmt.Errorf("install script from %s=%s is not a valid install script: %w", installStandaloneScriptEnv, path, err)
		}
		return script, nil
	}
...
func configureLoadBalancer(ctx context.Context, fullConfigRaw map[string]interface{}, networkName string, log log.Logger) ([]string, error) {
	...
	if reachable {
		...
		// the probe only matters on darwin, && short-circuits it away elsewhere
		canMountPrivilegedPorts := runtime.GOOS == "darwin" && canMountPrivilegedPort(ctx, log)
		if reason := loadBalancerUnsupportedReason(runtime.GOOS, reachable, canMountPrivilegedPorts, isPodmanBackend); reason != "" {
...
// loadBalancerUnsupportedReason returns the warning to log when load balancer
// support has to be disabled for the given platform and probe results, or an empty
// string when it can stay enabled. Pure so the platform branches, in particular the
// darwin podman machine one, can be unit tested on any OS.
func loadBalancerUnsupportedReason(goos string, networkReachable, canMountPrivilegedPorts, isPodman bool) string {
	if networkReachable {
		return ""
	}

	// forwarding through the loopback device only works on macos
	if goos != "darwin" {
		return "Load balancer type services are not supported inside the vCluster because the docker network is not reachable. Port-forwarding will not work. This is only supported on macOS"
	}

	if !canMountPrivilegedPorts {
		if isPodman {
			return "Load balancer type services are not supported inside the vCluster because Podman currently does not support mapping privileged ports to a specific host IP (see https://github.com/containers/podman/issues/28009)"
		}

		return "Load balancer type services are not supported inside the vCluster because privileged port mapping is not allowed. If you are using Docker Desktop, please enable it in the Docker Desktop settings"
	}
...
func getDockerSocketPath(ctx context.Context) (string, error) {
	...
	// therefore never shows up in the scan under that name. The docker.sock match is
	// deliberately narrow, since matching podman.sock would also pick up rootless
	// per-user sockets in /run/user, while the fallback list pins the rootful one.
	//
	// -S on its own is not enough in the fallback: a socket file left behind by a
	// removed daemon still passes it, which would pin the load balancer to a dead
	// docker.sock and never reach podman.sock. So each candidate is resolved through
	// its symlinks and required to be one of the sockets actually listening. The
	// listening set is read from both ss and netstat because minimal images usually
	// ship only one of them; when neither is available the -S test stands alone.
	cmdStr := `
        listening=""
        if command -v ss >/dev/null 2>&1; then
            listening=$(ss -xl 2>/dev/null | awk 'NR>1 {print $5}' | grep '^/' || true)
        fi
        if [ -z "$listening" ] && command -v netstat >/dev/null 2>&1; then
            listening=$(netstat -xl 2>/dev/null | awk '$NF ~ /^\// {print $NF}' || true)
        fi

        found=$(echo "$listening" | grep -E '/docker\.sock(\.real)?$' | head -n 1)
        if [ -n "$found" ]; then
            echo "$found"
            exit 0
        fi
        ...
        for p in /var/run/docker.sock.real /var/run/docker.sock /run/podman/podman.sock; do
            ...
            if [ -n "$listening" ]; then
                ...
                echo "$listening" | grep -qxF "$real" || continue
            fi
            echo "$p"
            exit 0
        done
	...
	out, err := hostNamespaceCommand(ctx, cmdStr).Output()
	if err != nil {
		var exitErr *exec.ExitError
		if errors.As(err, &exitErr) {
