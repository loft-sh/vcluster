...
func CreateHelm(ctx context.Context, options *CreateOptions, globalFlags *flags.GlobalFlags, vClusterName string, log log.Logger) error {
	...
	if isVClusterDeployed(release) {
		...
		if err := currentConfig.UnmarshalYAMLStrict([]byte(currentValues)); err != nil {
	...
	for _, p := range newExtraValues {
		...
		if err = newConfig.UnmarshalYAMLStrict(data); err != nil {
			if !errors.Is(err, config.ErrInvalidConfig) {
				return err
			}
			// TODO Delete after vCluster 0.19.x resp. the old config format is out of support.
		...
		// Only perform validation if there already is a virtual cluster running with config values.
		if isVClusterDeployed(release) && currentValues != "" {
			currentConfig := &config.Config{}
			if err = currentConfig.UnmarshalYAMLStrict([]byte(currentValues)); err != nil {
				return err
			}
		...
		if newConfig.Platform.API.AccessKey != "" || newConfig.Platform.API.SecretRef.Name != "" {
