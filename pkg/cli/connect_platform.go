...
func ConnectPlatform(ctx context.Context, options *ConnectOptions, globalFlags *flags.GlobalFlags, vClusterName string, command []string, log log.Logger) error {
	...
	kubeConfig, err := cmd.getVClusterKubeConfig(ctx, proClient, vCluster)
	if err != nil {
		return err
	}
...
func (cmd *connectPlatform) getVClusterKubeConfig(ctx context.Context, proClient platform.Client, vCluster *platform.VirtualClusterInstanceProject) (*clientcmdapi.Config, error) {
