...
// Originally from https://github.com/koding/websocketproxy
// Changes made: added Ping handler to connPub, which sends ping to connBackend
...
func (w *WebsocketProxy) ServeHTTP(rw http.ResponseWriter, req *http.Request) {
	...
	replicateWebsocketConn := func(dst, src *websocket.Conn, errc chan error) {
		for {
			...
			if err != nil {
				...
				if e, ok := lo.ErrorsAs[*websocket.CloseError](err); ok {
	...
	if e, ok := lo.ErrorsAs[*websocket.CloseError](err); !ok || e.Code == websocket.CloseAbnormalClosure {
