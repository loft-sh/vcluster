...
func ensureHostGatewayAPIKind(ctx *synccontext.RegisterContext, gvk schema.GroupVersionKind, configPath string) error {
	...
	if !exists {
		return fmt.Errorf("host cluster does not advertise Gateway API resource %s; install the Gateway API CRDs on the host cluster or disable %s", gvk.String(), configPath)
	}
