...
func getMappers(ctx *synccontext.RegisterContext) []BuildMapper {
	return append([]BuildMapper{
		...
		CreateEventsMapper,
		isEnabled(ctx.Config.Sync.FromHost.IngressClasses.Enabled, CreateIngressClassesMapper),
		isEnabled(ctx.Config.Sync.ToHost.Ingresses.Enabled, CreateIngressesMapper),
		isEnabled(ctx.Config.Experimental.MultiNamespaceMode.Enabled, CreateNamespacesMapper),
		...
		isEnabled(ctx.Config.Sync.ToHost.PodDisruptionBudgets.Enabled, CreatePodDisruptionBudgetsMapper),
