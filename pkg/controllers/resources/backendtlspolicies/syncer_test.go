...
func TestSyncSupportsSecretCACertificateRef(t *testing.T) {
	...
	spec.Validation.CACertificateRefs[0].Kind = gatewayv1.Kind("Secret")
	vPolicy := backendTLSPolicy(virtualPolicyMeta(), spec)
	syncCtx, syncer := startBackendTLSPolicySyncer(t, []runtime.Object{
		managedHostService(testServiceName, testPolicyNamespace),
		managedHostSecret(testConfigMapName, testPolicyNamespace),
	}, []runtime.Object{vPolicy.DeepCopy()})
	...
	err = syncCtx.HostClient.Get(syncCtx, syncer.VirtualToHost(syncCtx, types.NamespacedName{Name: vPolicy.Name, Namespace: vPolicy.Namespace}, vPolicy), storedHost)
	assert.NilError(t, err)
	assert.Equal(t, string(storedHost.Spec.Validation.CACertificateRefs[0].Name), hostName(testConfigMapName, testPolicyNamespace))
}
