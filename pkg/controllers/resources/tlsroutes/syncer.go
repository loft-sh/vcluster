...
func (s *tlsRouteSyncer) SyncToHost(ctx *synccontext.SyncContext, event *synccontext.SyncToHostEvent[*gatewayv1.TLSRoute]) (ctrl.Result, error) {
	if event.HostOld != nil || event.Virtual.DeletionTimestamp != nil {
		return patcher.DeleteVirtualObject(ctx, event.Virtual, event.HostOld, tlsRouteDeleteReason(event.Virtual))
	}
	...
	err = pro.ApplyPatchesHostObject(ctx, nil, pObj, event.Virtual, ctx.Config.Sync.ToHost.GatewayAPI.TLSRoutePatches, false)
	if err != nil {
		return ctrl.Result{}, err
	}
...
func (s *tlsRouteSyncer) Sync(ctx *synccontext.SyncContext, event *synccontext.SyncEvent[*gatewayv1.TLSRoute]) (_ ctrl.Result, retErr error) {
	...
	patch, err := patcher.NewSyncerPatcher(ctx, event.Host, event.Virtual, patcher.TranslatePatches(ctx.Config.Sync.ToHost.GatewayAPI.TLSRoutePatches, false))
	if err != nil {
...
func (s *tlsRouteSyncer) SyncToVirtual(ctx *synccontext.SyncContext, event *synccontext.SyncToVirtualEvent[*gatewayv1.TLSRoute]) (ctrl.Result, error) {
	...
	err := pro.ApplyPatchesVirtualObject(ctx, nil, vRoute, event.Host, ctx.Config.Sync.ToHost.GatewayAPI.TLSRoutePatches, false)
	if err != nil {
		return ctrl.Result{}, err
	}
...
func tlsRouteDeleteReason(route *gatewayv1.TLSRoute) string {
