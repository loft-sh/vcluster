...
func RegisterSyncer(ctx *context2.ControllerContext, eventBroadcaster record.EventBroadcaster) error {
	err := generic.RegisterSyncerIndices(ctx, &networkingv1.Ingress{})
	if err != nil {
		return err
	}

	return generic.RegisterSyncer(ctx, "ingress", &syncer{
		Translator: translator.NewNamespacedTranslator(ctx.Options.TargetNamespace, ctx.VirtualManager.GetClient(), &networkingv1.Ingress{}),
...
func (s *syncer) Forward(ctx context.Context, vObj client.Object, log loghelper.Logger) (ctrl.Result, error) {
	return s.creator.Create(ctx, vObj, s.translate(vObj.(*networkingv1.Ingress)), log)
}
