...
func TestSyncMirrorsGatewayClassFromPlainHostObject(t *testing.T) {
	hostClass := &gatewayv1.GatewayClass{
		ObjectMeta: metav1.ObjectMeta{
			Name:        "plain-gwc",
			Labels:      map[string]string{"app": "gateway"},
			Annotations: map[string]string{"gateway.example.com/source": "host"},
		},
		Spec: gatewayv1.GatewayClassSpec{
			ControllerName: gatewayv1.GatewayController("example.com/gateway-controller"),
		},
	}
	virtualClass := &gatewayv1.GatewayClass{
		ObjectMeta: metav1.ObjectMeta{
			Name:            "plain-gwc",
			Annotations:     map[string]string{"gateway.example.com/source": "virtual"},
			ResourceVersion: "999",
		},
	}

	pClient := testingutil.NewFakeClient(scheme.Scheme, hostClass.DeepCopy())
	vClient := testingutil.NewFakeClient(scheme.Scheme, virtualClass.DeepCopy())
	...
	_, err := syncer.(*gatewayClassSyncer).Sync(syncCtx, synccontext.NewSyncEvent(hostClass.DeepCopy(), virtualClass.DeepCopy()))
	assert.NilError(t, err)

	storedVirtual := &gatewayv1.GatewayClass{}
	err = syncCtx.VirtualClient.Get(syncCtx, types.NamespacedName{Name: "plain-gwc"}, storedVirtual)
	assert.NilError(t, err)
