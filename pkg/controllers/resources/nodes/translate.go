...
func (s *nodeSyncer) translateUpdateStatus(ctx *synccontext.SyncContext, pNode *corev1.Node, vNode *corev1.Node) error {
	...
	if s.enableScheduler {
		...
		if translatedStatus.Allocatable != nil {
			...
			err := s.unmanagedPodCache.List(ctx, unmanagedPods, client.MatchingFields{constants.IndexRunningNonVClusterPodsByNode: pNode.Name})
			if err != nil {
				return fmt.Errorf("error listing unmanaged pods: %w", err)
			}
			...
			if err != nil {
				return fmt.Errorf("error listing managed pods: %w", err)
			}
