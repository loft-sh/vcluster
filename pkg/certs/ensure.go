...
func warnIfCAExpiring(secretData map[string][]byte) {
	for _, secretKey := range certMap {
		...
		for _, cert := range certs {
			if certhelper.IsCertExpired(cert) {
				klog.Warningf("CA certificate %s (CN=%s) expires at %s; run %q to renew",
					secretKey, cert.Subject.CommonName, cert.NotAfter.Format(time.RFC3339), "vcluster certs rotate-ca")
			}
		}
	}
}
