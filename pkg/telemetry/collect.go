...
const (
	...
	telemetryEndpoint = "https://admin.loft.sh/analytics/v1/vcluster/v1/syncer"
)
...
func (d *DefaultCollector) executeUpload(buffer []*Event) {
	...
	if err != nil {
		d.log.Debugf("failed to json.Marshal telemetry requests: %v", err)

		return
	}

	marshaledReader := bytes.NewReader(marshaled)

	resp, err := http.Post(telemetryEndpoint, "multipart/form-data", marshaledReader)
	if err != nil {
