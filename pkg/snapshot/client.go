...
func (c *Client) List(ctx context.Context) ([]snapshot.Snapshot, error) {
	var err error
	// make sure to validate options
...
func (c *Client) writeEtcdSnapshot(ctx context.Context, etcdClient etcd.Client, objectStore types.Storage) error {
	...
	if c.Options.Release != nil {
		...
		err = writeArchiveEntry(tarWriter, []byte(snapshot.SnapshotReleaseKey), releaseBytes)
		if err != nil {
	...
	if c.Request != nil {
		...
		key := fmt.Sprintf("%s/%s", RequestStoreKey, snapshot.APIVersion)
