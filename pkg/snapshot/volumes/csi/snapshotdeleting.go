...
func (s *VolumeSnapshotter) reconcileDeleting(ctx context.Context, requestObj runtime.Object, requestName string, request *snapshot.VolumeSnapshotsRequest, status *snapshot.VolumeSnapshotsStatus) (retErr error) {
	if !status.IsDeletingVolumeSnapshots() {
		return fmt.Errorf("invalid phase for snapshot request %s, expected %s or %s, got %s", requestName, snapshot.VolumeSnapshotPhaseDeleting, snapshot.VolumeSnapshotPhaseCanceling, status.Phase)
	}
		...
		if volumeSnapshotStatus.IsVolumeSnapshotMaybeCreated() || volumeSnapshotStatus.IsDeletingVolumeSnapshot() {
			...
			if deletedResources {
				...
				s.eventRecorder.Eventf(
					...
					fmt.Sprintf("VolumeSnapshots%s", volumeSnapshotStatus.Phase),
					"%s volume snapshot for PVC %s/%s",
					status.Phase,
					volumeSnapshotRequest.PersistentVolumeClaim.Namespace,
					volumeSnapshotRequest.PersistentVolumeClaim.Name)
	...
	if !stillDeleting {
		...
		s.eventRecorder.Eventf(
			...
			corev1.EventTypeNormal,
			string(status.Phase),
			fmt.Sprintf("VolumeSnapshots%s", status.Phase),
			"%s volume snapshots",
			status.Phase,
		)
	}
