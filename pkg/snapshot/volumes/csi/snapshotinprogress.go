...
func (s *VolumeSnapshotter) reconcileInProgress(ctx context.Context, requestObj runtime.Object, requestName string, request *snapshotapi.VolumeSnapshotsRequest, status *snapshotapi.VolumeSnapshotsStatus) (retErr error) {
	...
	for _, volumeSnapshotRequest := range request.Requests {
		...
		if !ok {
			snapshotStatus = snapshotapi.VolumeSnapshotStatus{
				Phase: snapshotapi.VolumeSnapshotPhaseInProgress,
			}
		...
		case snapshotapi.VolumeSnapshotPhaseNotStarted:
			snapshotStatus.Phase = snapshotapi.VolumeSnapshotPhaseInProgress
			fallthrough
		case snapshotapi.VolumeSnapshotPhaseInProgress:
			...
			if newStatus.Phase == snapshotapi.VolumeSnapshotPhaseInProgress {
		...
		case snapshotapi.VolumeSnapshotPhaseFailedCleaningUp:
			...
			if cleanedUp {
				...
				status.Snapshots[pvcName] = snapshotStatus
				if snapshotStatus.Phase == snapshotapi.VolumeSnapshotPhaseFailed {
					failedSnapshotsCount++
				} else if snapshotStatus.Phase == snapshotapi.VolumeSnapshotPhaseCompleted {
...
func (s *VolumeSnapshotter) reconcileInProgressPVC(ctx context.Context, requestObj runtime.Object, requestName string, volumeSnapshotRequest snapshotapi.VolumeSnapshotRequest, volumeSnapshotStatus snapshotapi.VolumeSnapshotStatus) snapshotapi.VolumeSnapshotStatus {
	updatedStatus := func(err error) snapshotapi.VolumeSnapshotStatus {
		if err != nil {
			volumeSnapshotStatus.Phase = snapshotapi.VolumeSnapshotPhaseFailedCleaningUp
	...
	if volumeSnapshotStatus.Phase != snapshotapi.VolumeSnapshotPhaseInProgress {
		return updatedStatus(fmt.Errorf("invalid volume snapshot request phase %s for PVC %s, expected %s, got %s", volumeSnapshotStatus.Phase, pvcName.String(), snapshotapi.VolumeSnapshotPhaseInProgress, volumeSnapshotStatus.Phase))
	}
	...
	volumeSnapshotStatus.Phase = snapshotapi.VolumeSnapshotPhaseCompletedCleaningUp
	return volumeSnapshotStatus
}
...
func (s *VolumeSnapshotter) inProgressPVCReconcileFinished(requestObj runtime.Object, volumeSnapshotRequest snapshotapi.VolumeSnapshotRequest, volumeSnapshotStatus snapshotapi.VolumeSnapshotStatus, err error) {
