...
func mapCSIDriversToVolumeSnapshotClasses(ctx context.Context, snapshotsClient *snapshotsv1.Clientset, logger log.Logger) (map[string]string, error) {
	m := map[string]string{}

	volumeSnapshotClasses, err := snapshotsClient.SnapshotV1().VolumeSnapshotClasses().List(ctx, metav1.ListOptions{})
	if err != nil {
	...
	for _, volumeSnapshotClass := range volumeSnapshotClasses.Items {
		...
		logger.Debugf("Found VolumeSnapshotClass %q (with '%s' deletion policy) for CSI driver %q", volumeSnapshotClass.Name, volumeSnapshotClass.DeletionPolicy, volumeSnapshotClass.Driver)
	}
