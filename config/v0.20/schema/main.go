...
var (
	OutFile       = fmt.Sprintf("./config/%s/chart/values.schema.json", version)
	ValuesOutFile = fmt.Sprintf("./config/%s/chart/values.yaml", version)
)
...
func addPlatformSchema(toSchema *jsonschema.Schema) error {
	...
	if err := jsonschema.ExtractGoComments("github.com/loft-sh/vcluster", fmt.Sprintf("config/%s", version), commentsMap); err != nil {
		return err
	}
