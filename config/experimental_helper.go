...
var advisors = map[string]func(exp *ExperimentalConfig) (warning string){
	"sleepMode": SleepModeWaring,
}
...
func ExperimentalWarning(logger log.Logger, currentValues []byte) string {
	...
	for k := range exp.Experimental {
		if advisor, ok := advisors[k]; ok {
			if warning := advisor(exp); warning != "" {
...
func SleepModeWaring(exp *ExperimentalConfig) string {
	// if we're not upgrading to v0.24+ no warning
	if semver.Compare("v"+upgrade.GetVersion(), v24) == -1 {
		return ""
	}

	if _, ok := exp.Experimental["sleepMode"]; ok {
		return `
