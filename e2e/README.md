...
The e2e tests are checked by custom golangci-lint plugins (built via `golangci-lint custom` from `.custom-gcl.yml`). These run in CI and locally via `just lint ./e2e/...`.
...
```go
// vcluster-pro/e2e/suite_deploy_etcd_test.go
