...
// CSIHostpathPreSetup returns a PreSetupFunc that installs the CSI hostpath
// driver and snapshot infrastructure on the host cluster. Snapshot CRDs, the
// snapshot-controller and the driver are shared cluster-scoped infrastructure
// and are intentionally left installed after the suite finishes.
func CSIHostpathPreSetup() func(ctx context.Context) error {
	return InstallCSIHostpath("kind-" + constants.GetHostClusterName())
}
