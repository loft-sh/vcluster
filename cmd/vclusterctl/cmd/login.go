...
func (cmd *LoginCmd) Run(ctx context.Context, args []string) error {
	...
	if cmd.Manager != "" {
		err := use.SwitchManager(ctx, globalFlags.Config, cmd.Manager, log.GetInstance())
		if err != nil {
			return fmt.Errorf("switch manager failed: %w", err)
		}
	}

	cfg := config.Read(globalFlags.Config, cmd.Log)
	loginClient := platform.CreateLoginClientFromConfig(cfg.Platform.Config)
	var retErr error
	defer func() {
		// Write the latest config to file.
		platformConfig := loginClient.Config()
		cfg.Platform.Config = platformConfig
		if err := config.Write(cmd.Config, cfg); err != nil {
			retErr = fmt.Errorf("save vCluster config: %w", err)
		}
	}()
	...
	if len(args) == 0 {
		...
		if url == "" {
			...
			err := cmd.printLoginDetails(ctx, loginClient, cfg.Platform.Config)
			if err != nil {
	...
	err = dockerLogin(ctx, loginClient, cmd.Log)
	if err != nil {
		return err
	}
...
func (cmd *LoginCmd) printLoginDetails(ctx context.Context, loginClient platform.LoginClient, config platform.Config) error {
	if config.Host == "" {
		cmd.Log.Info("Not logged in")
		return nil
	}

	managementClient, err := loginClient.Management()
	if err != nil {
		return err
	}
...
func dockerLogin(ctx context.Context, loginClient platform.LoginClient, log log.Logger) error {
	managementClient, err := loginClient.Management()
	if err != nil {
		return err
	}
