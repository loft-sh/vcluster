...
func (cmd *DestroyCmd) Run(ctx context.Context) error {
	...
	if !cmd.NonInteractive {
		...
		if cmd.ForceRemoveFinalizers {
			...
			out, err := cmd.Log.Question(&survey.QuestionOptions{
				Question: fmt.Sprintf("IMPORTANT! You have selected the --force-remove-finalizers option. Please make sure you know what you're doing. Removing finalizers may cause unintended behaviours like leaving resources behind, but will ensure the platform is uninstalled. To confirm, please type %q", forceRemoveOpt),
			})
			if err != nil {
				return fmt.Errorf("failed to prompt for confirmation: %w", err)
			}
