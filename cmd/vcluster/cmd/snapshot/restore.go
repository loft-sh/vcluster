...
the backing store.`,
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			// restore proceeds.
			if active, state := isServiceActive(); active {
				return fmt.Errorf(
					"refusing to restore: %s.service is %s on this host; use the vcluster CLI (%q), which stops and restarts the service around the restore, or stop it first with %q",
