...
The vcluster syncer ensures that the intuitive naming logic of Kubernetes DNS names for services applies and users can connect to these DNS names which in fact map the the IP address of the synchronized services that are present in the underlying host cluster.
...
If you do **not** want ingresses to be synchronized and instead, you want to use a separate ingress controller within a vcluster, set the following in your `values.yaml`: 
```
sync:
  ingresses:
    enabled: false
