...
sidebar_label: Admission checks
---

Admission checks are a feature added to the vcluster API server proxy to block certain API requests despite users having cluster-admin permissions. It can deny requests that match any rule defined in the declarative configuration. In a future iteration of this feature, we plan to add an option for calling external webhooks for deciding on blocking, or allowing, the request.
