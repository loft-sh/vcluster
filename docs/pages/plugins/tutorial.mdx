...
In this tutorial we will implement a ConfigMap syncer. Vcluster syncs ConfigMaps out of the box, but only those that are used by one of the pods created in the vcluster. Here we will have a step-by-step look at a plugin implementation that will synchronize all ConfigMaps using the [vcluster plugin SDK](https://github.com/loft-sh/vcluster-sdk).
...
We can start our project by creating a `main.go` file:
```
package main
...
In this chapter, we will be writing the `sync-all-configmaps.go` file that we shall create in the `syncer` directory.
...
### Add go.mod and Dockerfile

Before you can build your plugins container image, you will need `go.mod` and `Dockerfile` files. You can find both of them and all the code from this tutorial in the example plugin project on GitHub - [github.com/loft-sh/vcluster-plugin-example](https://github.com/loft-sh/vcluster-plugin-example).
...
When developing your plugin we recommend using the [devspace](https://devspace.sh/) CLI tool for running your local plugin source code in Kubernetes. The appropriate configuration is present in our [example vcluster plugin project](https://github.com/loft-sh/vcluster-plugin-example), and you can read the instruction on how to use it in [its ReadMe](https://github.com/loft-sh/vcluster-plugin-example#readme).
