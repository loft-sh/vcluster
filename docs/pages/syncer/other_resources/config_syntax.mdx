...
- the RBAC namespaced `role` and cluster scoped `clusterRole` required for the plugin - these would be adjusted to fit the needs of your use case and the configuration that you define. Note that when the ["Multi-namespace mode"](./multi_namespace_mode) is used, the namespaced role will become ClusterRole.
