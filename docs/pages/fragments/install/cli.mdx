...
```bash
curl -L -o vcluster "https://github.com/loft-sh/vcluster/releases/download/latest/vcluster-darwin-amd64" && sudo install -c -m 0755 vcluster /usr/local/bin
```
...
```bash
curl -L -o vcluster "https://github.com/loft-sh/vcluster/releases/download/latest/vcluster-darwin-arm64" && sudo install -c -m 0755 vcluster /usr/local/bin
```
...
Invoke-WebRequest -URI "https://github.com/loft-sh/vcluster/releases/download/latest/vcluster-windows-amd64.exe" -o $Env:APPDATA\vcluster\vcluster.exe;
