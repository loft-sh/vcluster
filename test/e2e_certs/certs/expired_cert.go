...
var _ = ginkgo.Describe("vCluster cert rotation tests", ginkgo.Ordered, func() {
	...
	ginkgo.It("setting validity of ca cert of vCluster to 5 seconds", func() {
		os.Setenv("DEVELOPMENT", "true")
		os.Setenv("VCLUSTER_CERTS_VALIDITYPERIOD", "5s	")
	...
	ginkgo.It("should check if CA cert of vCluster is expired", func() {
		...
		gomega.Expect(cert.NotAfter.Before(time.Now())).To(gomega.BeTrue(), "CA cert is expired")
	})
