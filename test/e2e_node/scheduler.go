...
var _ = ginkgo.Describe("Scheduler sync", func() {
	...
	ginkgo.It("Use taints and toleration to assign virtual node to pod", func() {
		...
		_, err = f.VclusterClient.CoreV1().Pods("default").Create(f.Context, pod1, metav1.CreateOptions{})
		framework.ExpectNoError(err)

		err = wait.Poll(time.Second, time.Minute*2, func() (bool, error) {
			p, _ := f.VclusterClient.CoreV1().Pods("default").Get(f.Context, "nginx1", metav1.GetOptions{})
			if p.Status.Phase == v1.PodRunning {
				return true, nil
			}
			return false, nil
		...
		err = f.VclusterClient.CoreV1().Pods("default").Delete(f.Context, "nginx", metav1.DeleteOptions{})
		framework.ExpectNoError(err)

		err = f.VclusterClient.CoreV1().Pods("default").Delete(f.Context, "nginx1", metav1.DeleteOptions{})
		framework.ExpectNoError(err)
	})
})
