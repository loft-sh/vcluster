...
var _ = ginkgo.Describe("Persistent volume synced from host cluster", func() {
	...
	ginkgo.It("Test pvc provisioned successfully and is synced back to vcluster", func() {
		...
		pvc, err := f.HostClient.CoreV1().PersistentVolumeClaims(translate.Default.HostNamespace(ns)).Get(f.Context, translate.Default.HostName(pvcName, ns), metav1.GetOptions{})
		framework.ExpectNoError(err)
